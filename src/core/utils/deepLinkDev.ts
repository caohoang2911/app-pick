 ...
 */
export const testDeepLink = (path: string, params: Record<string, string> = {}, scheme: string = 'apppick'): void => {
  ...
  if (Platform.OS === 'ios') {
    ...
    console.log(`adb shell am start -a android.intent.action.VIEW -d "${url}" com.caohoang2911.AppPick`);
  }
};
...
export const debugDeepLinkSetup = (): void => {
  ...
  console.log('- Main scheme: apppick://');
  console.log('- Bundle ID: com.caohoang2911.seedcom-app-pick (iOS)');
  console.log('- Package: com.caohoang2911.AppPick (Android)');
  console.log('- Universal Links: https://oms.seedcom.vn/...');
  
  if (Platform.OS === 'ios') {
    console.log('\nFor iOS, make sure:');
    console.log('1. CFBundleURLTypes is correctly set in Info.plist');
    console.log('2. apppick is in the list of URL schemes');
    ...
    console.log('   - apppick scheme');
    console.log('   - com.caohoang2911.AppPick scheme');
