...
export const SafeScrollView = forwardRef<ScrollView, SafeScrollViewProps>(
  ({ children, onTouchStart, onTouchMove, onTouchEnd, onTouchCancel, ...props }, ref) => {
    // Safe touch event handlers that prevent crashes
    const safeOnTouchStart = useCallback((event: any) => {
      try {
        ...
        if (event && event.nativeEvent && event.nativeEvent.touches) {
          ...
          for (let i = 0; i < touches.length; i++) {
            if (touches[i].identifier < 0) {
              console.warn('Invalid touch identifier detected, ignoring touch event');
              return;
            }
          }
        }
    ...
    const safeOnTouchMove = useCallback((event: any) => {
      try {
        // Validate touch event before processing
        if (event && event.nativeEvent && event.nativeEvent.touches) {
          const touches = event.nativeEvent.touches;
          ...
          for (let i = 0; i < touches.length; i++) {
            if (touches[i].identifier < 0) {
              console.warn('Invalid touch identifier detected, ignoring touch move event');
              return;
            }
          }
        }
    ...
    const safeOnTouchEnd = useCallback((event: any) => {
      try {
        // Validate touch event before processing
        if (event && event.nativeEvent && event.nativeEvent.changedTouches) {
          ...
          for (let i = 0; i < changedTouches.length; i++) {
            if (changedTouches[i].identifier < 0) {
              console.warn('Invalid touch identifier detected, ignoring touch end event');
              return;
            }
          }
        }
    ...
    const safeOnTouchCancel = useCallback((event: any) => {
      try {
        // Validate touch event before processing
        if (event && event.nativeEvent && event.nativeEvent.changedTouches) {
          const changedTouches = event.nativeEvent.changedTouches;
          ...
          for (let i = 0; i < changedTouches.length; i++) {
            if (changedTouches[i].identifier < 0) {
              console.warn('Invalid touch identifier detected, ignoring touch cancel event');
              return;
            }
          }
        }
