...
const useHandleDeepLink = () => {
  ...
  function extractPathFromUrl(url: string): string {
    ...
    // Only handle apppick:// scheme
    if (url.startsWith('apppick://')) {
      // Remove the scheme and any trailing slashes
      const path = url.replace('apppick://', '').split('?')[0];
      console.log("[DeepLink] Extracted path from apppick:// URL:", path);
      return path;
    }
  ...
  function handleDeepLink(event: { url: string }) {
    try {
      ...
      // Only process apppick:// URLs in the dev environment
      ...
      if (__DEV__) {
        ...
        console.log("- Using scheme: apppick:// (Development)");
        console.log("- Bundle ID (iOS): com.caohoang2911.seedcom-app-pick");
        console.log("- Package (Android): com.caohoang2911.AppPick");
      }
