...
function routeByPath(path: string, orderCode?: string, deliveryCode?: string) {
  ...
  const routeMap: Record<string, () => void> = {
    ...
    [ROUTES.DEEP_LINK.ORDER_INVOICE]: () => {
      if (orderCode) {
        console.log(
          `[DeepLink] Routing to order invoice with code: ${orderCode}`,
        );
    ...
    [ROUTES.DEEP_LINK.SCAN_TO_DELIVERY]: () => {
      if (deliveryCode) {
        console.log(
          `[DeepLink] Routing to scan to delivery with code: ${deliveryCode}`,
        );
  ...
  if (orderCode) {
    console.log(
      `[DeepLink] Fallback: Routing to order pick with code: ${orderCode}`,
    );
    NavigationHelpers.replaceWithOrderPick(orderCode);
  } else if (deliveryCode) {
    console.log(
      `[DeepLink] Fallback: Routing to scan to delivery with code: ${deliveryCode}`,
    );
    NavigationHelpers.replaceWithScanToDelivery(deliveryCode);
  } else {
...
const useHandleDeepLink = () => {
  ...
  useEffect(() => {
    if (status === 'signIn') {
      ...
      if (pendingUrl) {
        console.log(
          '[DeepLink] Found pending deeplink after login:',
          pendingUrl,
        );
        setTimeout(() => {
          try {
            ...
            console.error(
              '[DeepLink] Error processing pending deep link:',
              error,
            );
          }
        }, 500);
    ...
    try {
      ...
      if (status === 'signOut') {
        if (isRedirectingToLogin.current) {
          console.log(
            '[DeepLink] Already redirecting to login, skipping duplicate redirect',
          );
          return;
        }
        ...
        try {
          console.log(
            '[DeepLink] User not authenticated, saving deep link for later',
          );
          ...
          console.error(
            '[DeepLink] Error handling unauthenticated deeplink:',
            error,
          );
          isRedirectingToLogin.current = false;
        }
  ...
  useEffect(() => {
    ...
    const initializeInitialUrl = async () => {
      try {
        ...
        if (url) {
          console.log(
            '[DeepLink] Processing initial URL after app start/reload:',
            url,
          );
          setTimeout(() => {
            handleDeepLink({ url });
