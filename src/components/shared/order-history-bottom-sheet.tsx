    ...
    return (
      <View key={index} className="flex-row items-start">
        ...
        <View className="relative w-6 mr-2">
          ...
          {!isLast && (
            <View
              className="absolute w-0.5 bg-gray-100"
              style={{ left: 5, top: 8, height: '100%' }}
            />
          )}
          {/* Blue timeline dot - align with first line of content */}
          <View
            className="absolute left-0 w-3 h-3 bg-blue-500 rounded-full z-10"
            style={{ top: 6 }}
          />
        </View>
            ...
            <View className="mt-1">
              {detailLines.map((line: string, lineIndex: number) => (
                <View key={lineIndex} className="flex-row items-start mb-0.5">
                  ...
                  <Text className="flex-1 text-sm text-gray-500">{line}</Text>
                </View>
              ))}
            </View>
          )}
