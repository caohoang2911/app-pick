...
const AssignStoreBottomSheet = forwardRef<
  ServingStoresModalRef,
  ServingStoresModalProps
>(({ driverAssignedStoreCodes }, ref) => {
  ...
  const stores = storesConfig.filter((store: Option) =>
    driverAssignedStoreCodes.includes(store.id as string),
  ) as Store[];
